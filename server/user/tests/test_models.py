...
class TestUserModel:
    ...
    def test_email_must_be_unique(self):
        ...
        with pytest.raises(IntegrityError):
            User.objects.create_user(email='duplicate@example.com', username='user2', password='pass')

    def test_username_must_be_unique(self):
        """Test that username must be unique."""
        User.objects.create_user(email='user1@example.com', username='uniqueuser', password='pass')
        with pytest.raises(IntegrityError):
            User.objects.create_user(email='user2@example.com', username='uniqueuser', password='pass')
